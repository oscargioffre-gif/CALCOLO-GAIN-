...
def build_jpeg(slots_mil, slots_nas) -> bytes:
    ...
    def draw_slot(col_x, idx, s, card_bg, border_col, sym, is_nas=False):
        ...
        # Slot number
        draw.text((tx, ty), f"· POSIZIONE {idx + 1}", font=fnt_slot, fill=MUTED)
        ty += 20
    ...
    def mil_slot(i):
        ...
        pct  = ((Pv * q) - Ka) / Ka * 100
        comm = Pv * q * 0.0019
        return dict(pc=pc, q=q, Ka=Ka, gl=gl, Pv=Pv, pct=pct, comm_v=comm)
    ...
    def nas_slot(i):
        ...
        Pv  = (gl + Ka + 9.0) / q
        pct = ((Pv * q) - Ka) / Ka * 100
        return dict(pc=pc, q=q, Ka=Ka, gl=gl, Pv=Pv, pct=pct, comm_v=9.0)
...
.slot-num{font-family:var(--mono);font-size:.62rem;letter-spacing:.18em;text-transform:uppercase;color:var(--muted);margin-bottom:.7rem}
...
def slot_milano(i):
    sym = "€"
    with st.container():
        st.markdown(f'<div class="slot-num">· Posizione {i}</div>', unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
        ...
        if pc > 0 and q > 0:
            ...
            gl = st.number_input(f"Guadagno lordo target {sym}", min_value=0.0,
            ...
            st.markdown(f'<div class="target-box"><div class="carico-label">🎯 Prezzo di vendita target</div>'
                        f'<span class="target-val">{sym} {Pv:,.4f}</span>'
                        f'<span class="badge-pct">+{pct:.2f}%</span></div>', unsafe_allow_html=True)
        else:
